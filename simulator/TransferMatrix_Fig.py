...
class OpticalModeling(object):
    ...
    def RunSim(self, thickness, show_fig=False):
        """
    ...
    def PlotE(self, savename="Result", savefig=False, figformat='pdf', 
        ...
        for matind in range(2, len(self.layers)+1):
            ...
            ax1.text(x_text, ax1.get_ylim()[1] + 0.01, self.layers[matind-1].split('_')[0],
                     size=14, va="bottom", ha="center")
            ax2.text(x_text, ax2.get_ylim()[1] + 0.01, self.layers[matind-1].split('_')[0],
                     size=14, va="bottom", ha="center")
    ...
    def PlotAbs(self, savename="Result", savefig=False, figformat='pdf'):
        ...
        for matind in range(1, len(self.t)):
            mater = self.layers[matind]
            mlabel = "L" + str(matind) + "_" + mater
            mlabel_txt = mater.split('_')[0]
            ax3.plot(self.WL, 100.0 * self.Absorption[mlabel], '--',
                     label=mlabel_txt,  linewidth=2)
    ...
    def PlotGen(self, savename="Result", savefig=False, figformat='pdf',
        ...
        for matind in range(2, len(self.layers)+1):
            ...
            ax4.text(x_text, ax4.get_ylim()[1] + 0.01, self.layers[matind - 1].split('_')[0],
                     size=14, va="bottom", ha="center")
            ax5.text(x_text, ax5.get_ylim()[1] + 0.01, self.layers[matind - 1].split('_')[0],
                     size=14, va="bottom", ha="center")
        fig4.tight_layout()
        fig5.tight_layout()
...
if __name__ == "__main__":
    ...
    if Demo: 
        ...
        OM = OpticalModeling(Device, 
                             WLrange=(350, 1200))
        OM.RunSim(thickness=[150, 50, 30, 44, 200])
